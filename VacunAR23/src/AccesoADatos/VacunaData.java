...
public class VacunaData {
    ...
    public void modificarVacuna(Vacuna vacu) {
        String sql = "UPDATE vacuna SET nombreVacuna = ?, marca = ?, medida = ?, vencimiento = ?, aplicacion = ? WHERE nroSerieDosis = ?";
        try (PreparedStatement ps = conex.Conexion_Maria().prepareStatement(sql)) {
            
            // Establecer la fecha de vencimiento como LocalDate
            
            //ps.setInt(1, vacu.getCuitLaboratorio());
            ps.setString(1, vacu.getNombreVacuna());
            ps.setString(2, vacu.getMarca());
            ps.setDouble(3, vacu.getMedida());            
            ps.setDate(4, Date.valueOf(vacu.getVencimiento()));
            ps.setBoolean(5, vacu.isAplicacion());
            
            ps.setInt(6, vacu.getNroSerieDosis());
    ...
    public Vacuna obtenerVacuna(int nroSerieDosis) {
        Vacuna vacuna = null;
        String sql = "SELECT * FROM vacuna WHERE nroSerieDosis = ? AND (aplicacion = 1 OR aplicacion = 0)";
