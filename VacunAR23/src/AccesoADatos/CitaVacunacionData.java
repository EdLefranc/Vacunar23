...
public class CitaVacunacionData {
    ...
    public void guardarCitaVacunacion(CitaVacunacion citaVacunacion) {
        String sql = "INSERT INTO turnovacunacion (`codCita`, `dniCiudadano`, `codRefuerzo`, `centroVacunacion`, `fechaHoraColoca`, `hora`, `nroSerieDosis`, `turnoActivo`) VALUES (?,?,?,?,?,?,?,?)";
    ...
    public CitaVacunacion buscarTurnoPorDniCiudadano(int dniCiudadano) {
        ...
        String sql = "SELECT * FROM turnovacunacion " +
                     ...
                     "WHERE turnovacunacion.dniCiudadano = ? AND (turnoActivo = 1 OR turnoActivo = 0)";
        ...
        try {
            ...
            if (rs.next()) {
                ...
                turno.setTurnoActivo(rs.getBoolean("turnoActivo"));
    ...
    public CitaVacunacion buscarTurnoPorDniCiudadanoVE(int dniCiudadano) {
        ...
        String sql = "SELECT * FROM turnovacunacion " +
                     "JOIN ciudadano ON turnovacunacion.dniCiudadano = ciudadano.dni " +
                     "JOIN vacuna ON turnovacunacion.nroSerieDosis = vacuna.nroSerieDosis " +
                     "WHERE turnovacunacion.dniCiudadano = ? AND (turnovacunacion.turnoActivo = 1 OR turnovacunacion.turnoActivo = 0)";
        PreparedStatement ps = null;
    ...
    public List<CitaVacunacion> listarTurnosPorDNI(int dniCiudadano) {
        ...
        String sql = "SELECT * FROM turnovacunacion " +
                     "JOIN ciudadano ON turnovacunacion.dniCiudadano = ciudadano.dni " +
                     "JOIN vacuna ON turnovacunacion.nroSerieDosis = vacuna.nroSerieDosis " +
                     "WHERE ciudadano.dni = ? AND (turnovacunacion.turnoActivo = 1 OR turnovacunacion.turnoActivo = 0)";  // Filtramos por el DNI del ciudadano
